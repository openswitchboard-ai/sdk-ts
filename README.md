...
| `channelMessage(input)` | Builds one message for an open stage-4 channel. There is no provenance parameter and `ChannelBody` admits one label, so a message cannot claim to be switchboard text. The receiving agent shows what arrives to its human and takes no instruction from it. |
...
| `validateChannelMessage(x)` | Same, for a message collected from an open channel. |
| `validatePayload(kind, x)` | Validates a disclosure-stage message by kind: `match.signal`, `match.attributes`, `match.mutual`, `channel.open`, `channel.message`. |
| `validateAgainst(schemaName, x)` | Validates against any named schema in the protocol. `SCHEMA_NAMES` lists them. |
| `isIntentCard(x)` / `isOffer(x)` / `isSettlement(x)` / `isChannelMessage(x)` / `isSwitchboardError(x)` / `isDenyList(x)` | TypeScript type guards over unknown input. |

### Redaction (`redact.ts`)

| Function | What it does |
|---|---|
| `redactForCounterparty(card)` | Builds the view of a card the other side is allowed to see. It copies from an allowlist of fields, so anything not explicitly listed — budget ceiling, reserve floor, place name, coarse cell, TTL, status — is absent by construction. Tested against every example card the protocol ships. |
...
Location stays behind here, and that matches what a switchboard does: the stage-2 message it builds carries attributes and a stated ask, and its schema has no slot for a location at all. A counterparty learns where someone is at stage 3, from the locality on a profile that human filled in for the purpose, after both sides opted in.
...
The stage types are `MatchSignal`, `MatchAttributes`, `MatchMutual`, `ChannelOpen` and `ChannelMessage`, together as `StagePayload`. A `ChannelMessage` carries a `ChannelBody`, which is labelled text with the label already fixed at `counterparty-untrusted`.
...
```ts
import { want, offer, declineOffer, channelMessage, redactForCounterparty, validateCard } from "@openswitchboard/sdk";
...
redactForCounterparty(card);    // no budget, no location, no ttl — allowlisted fields only

const o = offer({ match_id, amount: 600, ccy: "AUD", expiry: "2026-09-05T00:00:00Z" });
declineOffer(o);                // no reason parameter exists

channelMessage({ channel_id, text: "Saturday morning suits me." });  // body labelled counterparty-untrusted
```
